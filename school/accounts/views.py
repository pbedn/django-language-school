...
class StudentUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        student_form = self.student_form_class(
            request.GET or None, instance=student_instance
        )
        return render(
            request,
            self.template_name,
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        student_form = self.student_form_class(
            request.POST or None, instance=student_instance
        )
        if user_form.is_valid() and student_form.is_valid():
            user_form.save()
            student = student_form.save(commit=False)
...
class TeacherUpdateView(LoginRequiredMixin, View):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        teacher_form = self.teacher_form_class(
            request.GET or None, instance=teacher_instance
        )
        return render(
            request,
            self.template_name,
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        user_form = self.user_form_class(request.POST or None, instance=user_instance)
        teacher_form = self.teacher_form_class(
            request.POST or None, instance=teacher_instance
        )
        if user_form.is_valid() and teacher_form.is_valid():
            user_form.save()
            teacher = teacher_form.save(commit=False)
